...
def read_json_time_series(dict_data, time):
    ...
    if (time == 'miladi'):
        ...
        datelist = data.time.to_numpy()
        datearray = []
        for idx, x in np.ndenumerate(datelist):
            year, month, day = x.split('-')
            resultarray = jalali_to_gregorian(int(year), int(month), int(day))
            joined = str(resultarray[0]) + '-' + str(resultarray[1]) + '-' + str(resultarray[2])
            datearray.append(joined)
        data.time = pd.to_datetime(datearray)
